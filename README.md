...
- **Set run:** choose a saved list, run name, prompt, number of movies, response token limit, and concurrency (1–20). The count selects the first movies in saved list order. Use the standalone uppercase keyword `MOVIE` for the title and year, e.g. `Inception (2010)`. If the year is unknown, it uses the title alone. Legacy placeholders `{{MOVIE}}`, `{{YEAR}}`, and `{{IMDB_ID}}` remain supported for existing prompts. Preview the selection before clicking Start scoring run, which sends paid requests to OpenRouter. A cost estimate appears once reported request-cost history is available; it is an estimate and retries may increase it.
...
### MDBList columns, filters, and scoring

The downloaded records are indexed locally in `app/mdblist.sqlite3` and joined by IMDb ID. Use **Columns** to enable critic/audience ratings and their vote counts, MDBList score, budget/revenue, content guidance, keywords, services, language, country, certification, plot, posters, trailers, and dates. Ratings retain their native scales (Letterboxd /5, Metacritic users /10, Rotten Tomatoes /100). IMDb snapshot fields are not overwritten.

**Choose filters** provides numeric ranges and missing/present choices, multi-select categories, and an MDBList-data availability selector. Numeric comparisons exclude missing values; missing ratings display as a dash, not zero. Budget/revenue values reported as zero are treated as unknown. Streaming/provider labels reflect the cached source response, not a guarantee of a current subscription offer in your country.

Filter scoring favors higher ratings, votes, and financial values, and lower parental-content levels. Category scores use the share of selected values matched. Global scoring has independent weights plus higher/lower/target and fixed/percentile controls for every added numeric field. New weights default to zero, preserving existing rankings. Percentiles use known values; absent data uses the existing configurable missing-data score. Only downloaded movies have MDBList fields, so enabling these weights can favor that subset. No new API calls occur when browsing, filtering, or scoring.

Movie tabs have independent saved filters, search text, and filter-scoring settings. Existing `app/filters.json` remains the All movies profile. Starred, Seen, and Blocked start unfiltered and save into their own ignored `app/filters_<view>.json` files. Global scoring settings remain shared.

Global score settings support named presets: Save as new snapshots all current scoring controls; Load settings restores the snapshot for review, then Save & apply updates the ranking. Update selected explicitly replaces a preset, and Delete selected leaves current scoring unchanged. Presets persist locally in Git-ignored `app/global_presets.json`.

Named filter presets are available under **Saved filters** in the sidebar. Save as new captures the selected filters, values, search, and filter-score settings. Clear all removes all current filter selections (including Not seen) without changing saved presets. Load restores a preset into the current movie tab; Update selected explicitly replaces it. Presets are stored locally in Git-ignored `app/filter_presets.json`. Blocked movies remain excluded from the ordinary All view.
