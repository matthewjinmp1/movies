"""Named local snapshots of movie filter settings."""
import json
import os
import tempfile
import threading
...
import preferences
PATH=Path(__file__).resolve().parent/'filter_presets.json'
LOCK=threading.RLock()

def read():
    with LOCK:
        rows=json.loads(PATH.read_text()) if PATH.exists() else []
        return [dict(row,settings=preferences.validate(row['settings'])) for row in rows]
...
def change(payload):
    with LOCK:
        ...
        if action=='delete':rows.remove(existing)
            ...
            settings=preferences.validate(payload.get('settings'))
