"""Atomic local storage for movies marked as blocked."""
import json
import os
import re
import tempfile
...
PATH = Path(__file__).resolve().parent / 'blocked.json'
...
def read(path=None):
    ...
    with LOCK:
        ...
        if not isinstance(data, list) or any(not isinstance(v, str) or not re.fullmatch(r'tt[0-9]+', v) for v in data):
            raise ValueError('Invalid blocked movie list.')
        return set(data)
...
def update(movie_id, selected, path=None):
    ...
    if not isinstance(movie_id, str) or not re.fullmatch(r'tt[0-9]+', movie_id) or type(selected) is not bool:
        raise ValueError('Invalid blocked movie selection.')
    with LOCK:
        ids = read(path)
        if selected:
            ids.add(movie_id)
        else:
            ids.discard(movie_id)
        name = None
        try:
