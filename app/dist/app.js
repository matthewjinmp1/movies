...
function renderColumns(){$('column-options').innerHTML=meta.columns.map(c=>`<label><input type="checkbox" value="${c.key}" ${state.columns.includes(c.key)?'checked':''} ${state.columns.length===1&&state.columns[0]===c.key?'disabled':''}>${esc(c.label)}</label>`).join('');}
const FILTERS = [
...
$('movies-table').addEventListener('click',e=>{if(e.target.closest('[data-google-search]')){const scroller=document.querySelector('.table-scroll');history.replaceState({movieReturn:{page:state.page,scrollTop:scroller.scrollTop,scrollLeft:scroller.scrollLeft}},'');return;}const b=e.target.closest('[data-sort]');if(!b)return;state.direction=state.sort===b.dataset.sort?(state.direction==='asc'?'desc':'asc'):(['filterScore','globalScore'].includes(b.dataset.sort)?'desc':'asc');state.sort=b.dataset.sort;syncSort();refresh();});
...
function go(page){if(!data)return;clearTimeout(debounce);state.page=Math.max(1,Math.min(data.pages,page||1));load();document.querySelector('.table-scroll').scrollTop=0;}
