...
function renderGlobalSettings(){
 ...
 $('global-fields').innerHTML=Object.entries(s.fields).map(([key,f])=>`<fieldset data-global-field="${key}"><legend>${GLOBAL_FIELDS[key]}</legend><div class="scoring-grid"><label>Preference<select name="${key}-mode"><option value="higher">Favor higher values</option><option value="lower">Favor lower values</option><option value="target">Favor a target value</option></select></label><label>Score scale<select name="${key}-scale"><option value="percentile">Percentile across the library</option><option value="fixed">Fixed anchors / target distance</option></select></label><div data-global-part="anchors">${globalNumber(key+'-low','Low anchor',f.low,0,key==='averageRating'?10:key==='startYear'?9999:1000000000)}${globalNumber(key+'-high','High anchor',f.high,0,key==='averageRating'?10:key==='startYear'?9999:1000000000)}<small>Higher preference: low scores 0, high scores 100. Lower preference reverses these scores.</small></div><div data-global-part="target">${globalNumber(key+'-target','Target value',f.target,0,key==='averageRating'?10:key==='startYear'?9999:1000000000)}</div><div data-global-part="tolerance">${globalNumber(key+'-tolerance','Distance from target for score 0',f.tolerance,0.01,1000000000,'any')}<small>Target scores 100. Scores fall evenly to 0 at this distance in either direction.</small></div></div></fieldset>`).join('');
...
async function initGlobalSettings(){
 ...
 globalSettings=result.settings;globalDefaults=result.defaults;renderGlobalSettings();
